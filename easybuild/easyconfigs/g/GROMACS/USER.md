...
!!! Bug "Severe issues with GROMACS after the January 2026 system update"
    ...
    incorrect results could also happen.
...
-   There is also a HIP port of the CUDA version, intially developed by AMD but now included in 
    GROMACS from 2026.0 

 The GROMACS developers plan to
...
has now been largely closed. Despite this provide the HIP version as an alternative
as it is now an official part of the GROMACS distribution so that users can compare what
works best for their case. Currently, this also implies that it is important to try to switch
to the most recent versions of GROMACS for GPU, i.e., the 2026.0 EasyConfigs that we offer, rather 
...
As GROMACS 2025 the official version should contain all necessary parts of PLUMED that are needed when running GROMACS,
